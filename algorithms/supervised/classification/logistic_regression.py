...
class BinaryClassLogisticRegression:
    ...
    Scaling Techniques:
        ...
        1. Standardization:     - For dataset that has Normal or Gaussian Distribution
                                - Rescale the data around Mean of 0, and Standard Deviation of 1
                                - Useful when dataset has outliers
                                - formula: Z = Xnew = (X - Mean) / std
                                - Scikit Learn: 
                                    scaler = StandardScaler()
                                    standardized_x = scaler.fit_transform(x)

        2. Normalization:   - transform data to fit between 0 and 1, a comparable range so that no data will overshadow the other data
                            - Useful when scale of data varies significantly like Age, Salary, # of Children
    ...
    def __init__(self, visualize=True):
        self.df = pd.read_csv("../../../../files/final_files/DATA/hearing_test.csv")
...
class MultiClassLogisticRegression:
    def __init__(self):
        self.df = pd.read_csv("../../../../files/final_files/DATA/iris.csv")
