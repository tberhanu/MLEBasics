...
def report():
    ...
    # X_train_tfidf, X_test_tfidf, y_train, y_test = tfidf.train_test_split_and_tfidf()

    nb = MultinomialNB()
    nb.fit(tfidf.X_train_tfidf, tfidf.y_train)


    preds = nb.predict(tfidf.X_test_tfidf)
    print("Naive Bayes Model Report")

    print(classification_report(tfidf.y_test, preds))


    disp = ConfusionMatrixDisplay.from_estimator(nb, tfidf.X_test_tfidf, tfidf.y_test)
    disp.plot()
    plt.show()
